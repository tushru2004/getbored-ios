...
private let logger = Logger(
    ...
    final class AccountModule: NSObject {
        ...
        /**
         * Deletes the account remotely, then clears local credentials and applies
         * an empty policy on the main actor so the filter cannot retain deleted
         * account rules.
         *
         * Call flow:
         *
        ...
        @objc func deleteAccount(
            ...
            Task {
                do {
                    ...
                    Self.rejectRequest(
                        reject,
                        dueTo: APIError.normalized(error)
                    )
                }
            }
        }
